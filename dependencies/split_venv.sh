...
# Virtual Environment Archive and Split Script for GitHub
# Creates a compressed archive of the virtual environment and splits it for GitHub upload
...
echo -e "${BLUE}=== Virtual Environment Archive and Split for GitHub ===${NC}"
...
# Remove old archives that might be lying around
rm -f "./comparatron_env.tar.gz" "comparatron_env_main.tar.gz" 2>/dev/null
...
rm -f "$SPLITS_DIR/comparatron_env_main.tar.gz" 2>/dev/null
rm -f "$SPLITS_DIR/comparatron_env_part_"* 2>/dev/null
rm -f "$SPLITS_DIR/split_info.txt" 2>/dev/null
...
echo -e "${YELLOW}Archiving and splitting virtual environment...${NC}"

# Create a compressed tar of the virtual environment (from parent directory)
MAIN_TAR="$SPLITS_DIR/comparatron_env_main.tar.gz"
echo -e "${YELLOW}Creating main archive...${NC}"

# Change to parent directory to create the relative path archive correctly
cd ..
tar -czf "dependencies/$SPLITS_DIR/comparatron_env_main.tar.gz" "comparatron_env"
cd dependencies
...
echo -e "${GREEN}=== Virtual environment archived and split successfully ===${NC}"
